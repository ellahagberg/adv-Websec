#!/usr/bin/python3
# -*- coding: utf-8 -*-

"""DH kex client."""

import os
import socket
import binascii

client = None


def generate_random(N, bits=1536):
    """Generate a random number with bits=1536, then mod N."""
    byte = bits // 8
    rnd_num = os.urandom(byte)
    # ensure the number of bits is as high as 'bits'
    while rnd_num[0] < 128:
        rnd_num = os.urandom(byte)

    # convert from ascii to hex to a decimal integer
    rnd_num = int(''.join(format(i, 'x') for i in rnd_num), 16)
    return rnd_num % N


def set_client(sock):
    """Set module-level client socket used by send/recv helpers."""
    global client
    client = sock


def send_int(num):
    """Send integer, 'num' is decimal number."""
    num_str = format(num, 'x').encode('utf8')
    client.sendall(num_str)


def recv_int():
    """Receive and convert it to integer."""
    num_str = client.recv(8192).decode('utf8').strip()
    return int(num_str, 16)


def recv_text():
    """Receive a textual message such as 'ack'/'nak' or final response."""
    return client.recv(8192).decode('utf8').strip()


if __name__ == '__main__':
    """python client.py"""
    p = int('', 16)
    # the generator of the group (Z/pZ)*
    g = 2
    # connect
    host = "0.0.0.0"
    host = "igor.eit.lth.se"
    client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    client.connect((host, 6004))
    set_client(client)
    # Add your own code for DH kex

    g_x1 = recv_int()

   
    x2 = generate_random(p)
    g_x2 = pow(g, x2, p)
    send_int(g_x2)

    #comp shared key
    key = pow(g_x1, x2, p)

    # here is decryption
    enc = recv_int()
    m_int = enc ^ key

    #int->bytes->decode
    m_hex = format(m_int, 'x')
    if len(m_hex) % 2 == 1:
        m_hex = '0' + m_hex
    plaintext = binascii.unhexlify(m_hex).decode('utf-8')

    print(plaintext)